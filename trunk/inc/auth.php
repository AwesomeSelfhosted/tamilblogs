...
function login($user,$pass) {
	...
	getUserName($result[0]['id']);
	return $result[0]['id'];
}

function logout() {
	setCookie('tamilblogs_auth','',time()+60*60*24*365);
	setCookie('tamilblogs_uid','',time()+60*60*24*365);
	return 0;
}

function checkAuthToken() {
	$uid = $_COOKIE['tamilblogs_uid'];
	$rnd = $_COOKIE['tamilblogs_auth'];
...
function setAuthToken($uid) {
	...
	$sql = "UPDATE tamilblogs_users
                   SET magic = '$rnd'
                 WHERE id = $uid";
	runSQL($sql);
	setCookie('tamilblogs_auth',$rnd,time()+60*60*24*365);
	setCookie('tamilblogs_uid',$uid,time()+60*60*24*365);
}
...
function checkUserName($name) {
	...
	$sql = "SELECT login
                 FROM tamilblogs_users
                WHERE login = '".addslashes($name)."'";
	$result = runSQL($sql);
	if($result[0]['login'] == $name){
		return 1;
	}
	return 0;
}
